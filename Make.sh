...
function debug() {
    ...
    dotnet build "$BASE_DIRECTORY/src/Influx.sln" \
                 ...
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/debug/Medo.Influx.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/debug/Medo.Influx.pdb" "$BASE_DIRECTORY/bin/" || return 1
...
function release() {
    ...
    dotnet build "$BASE_DIRECTORY/src/Influx.sln" \
                 ...
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/release/Medo.Influx.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/release/Medo.Influx.pdb" "$BASE_DIRECTORY/bin/" || return 1
    echo "${ANSI_CYAN}Output in 'bin/'${ANSI_RESET}"
}
